...
export default function RiderPickupList() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setPickups(
          tasks.filter((t) => t.type === "pickup" || t.taskType === "pickup").map((t) => ({
            id: t.orderId ?? t._id ?? t.id,
            customer: t.customerName ?? t.user?.name ?? "—",
            address: t.address?.street ?? t.pickupAddress ?? "—",
            slot: t.slot ?? t.timeSlot ?? "—",
            items: t.items ? `${t.items} bags` : "—",
            status: t.status ?? "Pending",
            statusColor: (t.status ?? "Pending") === "Pending" ? "bg-orange-100 text-orange-700" : t.status === "En Route" ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-600",
            icon: "light_mode",
          }))
        );
      } catch {
  ...
  const statCards = [
    { label: "Total Pickups", value: String(pickups.length), icon: "inventory_2", color: "bg-blue-50 text-primary" },
    { label: "Pending", value: String(pickups.filter((p) => p.status === "Pending").length), icon: "pending", color: "bg-orange-50 text-orange-700" },
    { label: "En Route", value: String(pickups.filter((p) => p.status === "En Route").length), icon: "local_shipping", color: "bg-cyan-50 text-secondary" },
    { label: "Scheduled", value: String(pickups.filter((p) => p.status === "Scheduled").length), icon: "schedule", color: "bg-slate-50 text-slate-600" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 ${loading ? "animate-pulse" : ""}`}>
        {statCards.map((s) => (
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="text-center py-8 text-on-surface-variant">Loading pickups…</div>
        ) : pickups.length === 0 ? (
          <div className="text-center py-8 text-on-surface-variant">No pickups assigned</div>
        ) : (
          pickups.map((p) => (
            <div key={p.id} className="glass-card rounded-xl p-5 hover:shadow-lg transition-shadow flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex items-center gap-4 flex-1">
                <div className="p-2.5 bg-white/30 rounded-lg"><span className="material-symbols-outlined text-secondary">{p.icon}</span></div>
              ...
              <div className="flex items-center gap-6 text-sm">
                <div className="text-center"><p className="text-[10px] text-on-surface-variant uppercase font-bold">Slot</p><p className="font-semibold text-on-surface">{p.slot}</p></div>
                <div className="text-center"><p className="text-[10px] text-on-surface-variant uppercase font-bold">Items</p><p className="font-semibold text-on-surface">{p.items}</p></div>
