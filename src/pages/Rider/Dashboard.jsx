  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const pickups = tasks.filter(
          (t) => t.type === "pickup" || t.taskType === "pickup",
        ).length;
        const deliveries = tasks.filter(
          (t) => t.type === "delivery" || t.taskType === "delivery",
        ).length;
        setStats({
          pickups: String(pickups),
          deliveries: String(deliveries),
          subscribers: String(tasks.length),
        });
      } catch {
  ...
  const actionCards = [
    {
      ...
      icon: "local_shipping",
      description:
        "Manage pending laundry collections, optimize routing, and mark bags as received.",
      cta: "View List",
    ...
    },
    {
      ...
      icon: "inventory_2",
      description:
        "Track clean laundry dispatches, confirm drop-offs, and manage delivery exceptions.",
      cta: "View List",
