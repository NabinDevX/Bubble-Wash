...
export default function RiderDeliveryList() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setDeliveries(
          tasks
            .filter((t) => t.type === "delivery" || t.taskType === "delivery")
            .map((t) => ({
  ...
  async function updateStatus(orderId, newStatus) {
    try {
      ...
      notify.error(err?.message || "Failed to update delivery status");
    }
  }
  ...
  const statCards = [
    ...
    {
      label: "In Transit",
      value: String(deliveries.filter((d) => d.status === "In Transit").length),
      icon: "delivery_dining",
      color: "bg-cyan-50 text-secondary",
    },
    {
      label: "Avg. ETA",
      value:
        deliveries.length > 0
          ? `${Math.round(deliveries.reduce((a, d) => a + (parseInt(d.eta) || 0), 0) / deliveries.length)} min`
          : "—",
      icon: "schedule",
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          deliveries.map((d) => (
            <div
              key={d.id}
              className="glass-card rounded-xl p-5 hover:shadow-lg transition-shadow flex flex-col sm:flex-row sm:items-center gap-4"
            >
              <div className="flex items-center gap-4 flex-1">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${d.statusColor}`}
                    >
                      {d.status}
                    </span>
                    {d.priority === "Express" && (
              ...
              <div className="flex items-center gap-6 text-sm">
                ...
                {d.status !== "Delivered" && (
                  <button
                    ...
                    className="px-3 py-1.5 text-xs font-semibold text-white bg-secondary rounded-lg hover:bg-secondary/80 transition-colors"
                  >
                    Mark Done
                  </button>
                )}
