...
export default function DeliverySlots() {
      ...
      try {
        ...
        setSlots(
          list.map((s) => ({
            ...
            label: s.isActive === false ? "Inactive Slot" : (s.booked ?? 0) / (s.capacity ?? 20) >= 0.8 ? "Almost Full" : "Available",
            labelColor: s.isActive === false ? "text-outline" : (s.booked ?? 0) / (s.capacity ?? 20) >= 0.8 ? "text-error" : "text-secondary",
          }))
        );
      } catch {
        // keep empty
      ...
      setSlots((prev) =>
        prev.map((s) => {
          ...
          return {
            ...s,
            active: newActive,
            label: !newActive ? "Inactive Slot" : s.booked / s.total >= 0.8 ? "Almost Full" : "Available",
            labelColor: !newActive ? "text-outline" : s.booked / s.total >= 0.8 ? "text-error" : "text-secondary",
          };
        })
      );
    } catch (err) {
  ...
  async function handleAddSlot() {
    ...
    try {
      await api.post("/admin/slots", { startTime, endTime, capacity: Number(capacity) });
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleAddSlot} className="px-5 py-2 rounded-lg text-white text-sm font-semibold bg-linear-to-r from-secondary-fixed-dim to-secondary shadow-md hover:shadow-lg transition-all flex items-center gap-2">
            ...
            return (
              <div key={slot.id} className={`glass-card rounded-xl p-5 flex flex-col group hover:shadow-lg transition-shadow ${!slot.active ? 'opacity-70 grayscale-[0.2]' : ''}`}>
                ...
                <div className="mt-5 pt-4 border-t border-outline-variant/30 flex justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
                  ...
                  <button onClick={() => handleDeleteSlot(slot.id)} className="p-2 text-outline hover:text-error hover:bg-red-50/50 rounded-lg transition-colors" aria-label="Delete Slot">
